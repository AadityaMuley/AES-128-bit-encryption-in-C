#include <stdio.h>
int rounds=1;
int statearray[4][4]={{0x0a,0x0b,0x0c,0x0d},{0x0a,0x0b,0xc,0x0d},{0x0a,0x0b,0x0c,0x0d},{0x0a,0x0b,0x0c,0x0d}};
int word[44][4];
int key[4][4];
...
{		
		...
		for(int i=0;i<3;i++)
			{word[i][3]=word[i+1][3];
			 word[i][3]=sbox[word[i][3]>>4][word[i][3]&0x0f];
			}	
		word[3][3] = t;
		word[3][3]=sbox[word[3][3]>>4][word[i][3]&0x0f];
		int RC[10] = {0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36};
		word[0][3]^=RC[rounds-1];
